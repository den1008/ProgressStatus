<?php
	...
	class StatusProcessor
	{
		...
		/**
		 * @var int Шаг (в процентах) с которым обрабатываются сообщения попавшие в метод say(). Если 0, то
		 *      обрабатываются все сообщения Например, если $stepViewProgress == 10, то обрабатываться будут сообщения
		 *      только с 10.12, 20.56, 30.98, ..., 100 процентами, т.е. остаток от деления на $stepViewProgress меньше
		 *      1. Если будет несколько сообщений подходящими под условие, например,
		...
		private function calcPercent($total, $progress)
		{
			//Если общее количество элементов равно 0, то считаем процент выполнения равным 100%
		 ...
		 *      //Подпрограмма ничего не знает о родительском статусе выполнения, поэтому ведет собственный расчет от 0
		 *      до {%total%}
		 ...
		 *          //Здесь параметры {%progress%} из {%total%} будут для текущего контекста, но {%commonPercent%}% -
		 *          для общего
		...
		{
			if($this->total < 0){
				throw new Exception("Максимальный прогресс внешней подпрограммы должен быть положительным числом");
			}
			$context              = [
				'total'       => $this->total,
		...
		{
			if($total < 0){
				throw new Exception("Общее количество должно быть положительным числом");
			}
			$this->total = $total;
			return $this;
		}
		...
		public function setProgress($progress)
		{
			...
			$this->percent     = $this->calcPercent($this->total, $progress);
		...
		public function getCommonPercent()
		{
			...
			foreach ($this->subProcesses as $subProcess){
				$endPercent    = $parentPercent + $this->calcPercent($subProcess['total'], $subProcess['end']) * $stepPercent;
				$parentPercent += $stepPercent * $this->calcPercent($subProcess['total'], $subProcess['progress']);
				$stepPercent   *= ($endPercent - $parentPercent) / 100;
			}

			return $parentPercent + $stepPercent * $this->calcPercent($this->total, $this->progress);
		}
	}
